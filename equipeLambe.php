...
<body>
    ...
    <?php require_once('templates/alertAdmin.php'); ?>
        ...
        <?php
            $query = $pdo->prepare("SELECT * FROM equipe WHERE nom_equipe = 'u19' OR nom_equipe = 'u16' OR nom_equipe = 'u18'");
            $query->execute();
            		...
            		<tbody>
						...
						<tr>
						    <fieldset><h4 class="mt-80">
						        <form action="edition_equipe.php?page_prec=equipeLambe.php" method="post">
						...
						<?php
                  			...
                  			for ($i = 0; $rowresponsable = $queryresponsable->fetch(); $i++) {
                    				if($i == 0) {
                    				    if($_SESSION['admin'] == true) {
                    				        $th = '<form action="edition_responsables.php?page_prec=equipeLambe.php" method="post">
                    		...
                    		if($i == 0) {
                    		    echo '<tr><td><form action="edition_responsables.php?page_prec=equipeLambe.php" method="post">
                                                    <input type="hidden" name="id" value="'.$row['nom_equipe'].'" />
                                                    <button class="btn btn-default" type="submit">Responsables <i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
                                                </form></td></tr>';
                    		}
            ...
            <fieldset>
                <h4>
                    Joueurs
                    <?php if($_SESSION['admin'] == true) { ?>
                    <form class="float-right" action="edition_joueurs.php?page_prec=equipeLambe.php" method="post">
                        <input type="hidden" name="id" value="<?php echo $row['nom_equipe']; ?>" />
